...
getPostsBtn.addEventListener('click', async() => {
    loader.style.display = 'block'; // Показываем лоадер перед загрузкой
...
postUser.addEventListener('input', () => {
    postUser.value = postUser.value.replace(/\D/g, '');
});

postPost.addEventListener('input', () => {
    postPost.value = postPost.value.replace(/\D/g, '');
});
